...
//                .getParent();
  ...
  /*  private void clear() {
    ...
    private class WriteDb extends Thread {
        ...
        @Override
        public void run() {
            logger_inner.info(getName() + " start...");
            try {
                switch (type) {
                    case list:
                        list(getName());
                    ...
                    case hash:
                        ...
                        break;
                    default:
                        logger_inner.error(type.toString() + " is not support...");
                }
            } catch (Exception e) {
                logger_inner.error(e);
            }
            downLatch.countDown();
        }
        ...
        private void kv_set_hash(String name) {
            ...
            for (int i = 0; i < _total; i++) {
                ...
                if ((System.currentTimeMillis() - start) > counter) {
                    logger_inner.info(name + "[" + type.toString() + ",key=>]" + _key);
                    counter += period;
                }
            ...
            logger_inner.info(name + "[" + type.toString() + ",耗时:=>]" + use_time + "[eps=>]" + _total / use_time);
        }
    ...
    private class QueryDb {
        ...
        private void query(Object... params) {
            switch (type) {
                ...
                case hash:
                    ...
                    break;
                default:
                    logger.error(type.toString() + " is not support...");
            }
        }
        ...
        private void list(Object... params) {
            ...
            long use_time = (System.currentTimeMillis() - start) / 1000;
            logger.info("[" + type.toString() + " qget 耗时=>]" + use_time);
            outputResp(type, response, params);
        }
        ...
        private void set_hash(Object... params) {
            ...
            long use_time = (System.currentTimeMillis() - start) / 1000;
            logger.info("[" + type.toString() + " zget/hget/multi_zget/multi_hget 耗时=>]" + use_time);
            outputResp(type, response, params);
        }
    }
    ...
    private void sumLogTime() {
        ...
        logger.info("[total time=>]" + total + "[threads=>]" + counter + "[avg time=>]" + (total / counter));
    }
