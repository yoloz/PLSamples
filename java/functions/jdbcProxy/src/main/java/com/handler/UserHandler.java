...
public class UserHandler {
    ...
    static void authSql(WrapConnect connect, String user, String sql) throws SQLException {
        ...
        parser.setDefaultDbName(connect.getDbName());
        parser.setConn(connect.getDbConnect());
        List<SqlInfo> sqlInfoList = parser.parseToSQLInfo();
        for (SqlInfo sqlInfo : sqlInfoList) {
            authSql(userPriv, connect.getDbKey(), user, sqlInfo);
        }
    }
     ...
     */
    private static void authSql(Object userPriv, String dbkey, String user, SqlInfo... sqlInfos) throws SQLException {
            ...
            for (String operator : operators) {
                if (!userAuth.contains(operator)) {
                    ...
                    if (!dbAuth.contains(operator)) {
                        ...
                        if (!tbList.contains(operator)) throw new SQLException("用户[" + user + "]无权限操作[" +
                                operator + "]表[" + tbName + "]");
                    }
                }
    ...
    private static void authColumn(Set<String> operators, List<String> userAuth, List<String> dbAuth,
                                   ...
                                   String dbkey, String user, String tbName, String colName) throws SQLException {
        for (String operator : operators) {
                    ...
                    if (!dbAuth.contains(operator)) {
                        ...
                        if (!tcList.contains(operator)) {
                            if (!colAuth.contains(operator))
                                throw new SQLException("用户[" + user + "]无权限操作[" + operator + "]列[" +
