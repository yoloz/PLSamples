...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        options.addOption(Option.builder("pro").longOpt("protocol").required().hasArg().desc("协议[sftp,ftp]").build());
        ...
        options.addOption(Option.builder("prt").longOpt("port").required().hasArg().desc("端口").type(Number.class).build());
        ...
        options.addOption(Option.builder("oder").longOpt("order").hasArg().desc("文件名[fname]或修改时间[mtime]增量下载").build());
        ...
        try {
            ...
            if ("sftp".equals(cmd.getOptionValue("pro"))) {
                ...
                abstractFtp = new Ftp(cmd.getOptionValue("usr"), cmd.getOptionValue("pwd"), cmd.getOptionValue("hst"),
                        ((Number) cmd.getParsedOptionValue("prt")).intValue(), cmd.getOptionValue("lp"),
                        cmd.getOptionValue("rp"), cmd.getOptionValue("oder", ""),
                        cmd.getOptionValue("sid", ""), cmd.hasOption("pa"), cmd.hasOption("epsv"),
            ...
            System.err.println("Missing required option:" + Arrays.toString(e.getMissingOptions().toArray()));
            HelpFormatter helper = new HelpFormatter();
            helper.printHelp("jsch.sh options...", options);
            System.exit(1);
        }
